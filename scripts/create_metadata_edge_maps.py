...
def severity_to_prompt(severity):
    ...
    """
    # Standardized format: "wheat head, [category] infection, X% severity"
    if severity == 0:
        return "AINBN3tpDb, no Blight, 0.0%"
    elif 0 < severity <= 10:
        return f"AINBN3tpDb, minimal Blight, {severity:.1f}%"
    elif 10 < severity <= 30:
        return f"AINBN3tpDb, mild Blight, {severity:.1f}%"
    elif 30 < severity <= 60:
        return f"AINBN3tpDb, moderate Blight, {severity:.1f}%"
    elif 60 < severity <= 100:
        return f"AINBN3tpDb, severe Blight, {severity:.1f}%"
    else:
        return f"AINBN3tpDb, unknown Blight, {severity:.1f}%"
...
def create_conditioning_filename(image_filename):
    ...
    Returns:
        str: Conditioning image filename with _edges suffix
    """
    base_name = Path(image_filename).stem
    return f"{base_name}_edges.png"
...
def main():
    ...
    parser.add_argument(
        ...
        type=str,
        required=True,
        help="Directory containing the conditioning/edge images"
    )
    parser.add_argument(
        "--output_file",
        type=str,
